...
# import numpy as np
import utility as np # note: kept np format & linalg + random was changed from np._ to np.norm or np.random
import h5py
...
class Simulation:
    ...
    #     x /= np.norm(x)
    #     return x
    # NORMALIZE V2; removing the temp array x 
    def normalize(self, x):
        np.divide(x, np.norm(x), out=x)
        return x
    ...
    #     ret = np.sqrt(np.sum(corner_pt[0:2]**2)) < self.T1_radius # 2 arrays 
    #     if scint_num == 4:
    #         ret = np.sqrt(np.sum(corner_pt[0:2]**2)) < self.T4_radius # 2 arrays
    #     return ret

    # SCINT RADIUS CONDITION V2 remove the temp array generated by sqrt 
    # def scint_condition(self, corner_pt, scint_radius, scint_num):
    #     radius_squared = corner_pt[0]**2 + corner_pt[1]**2 # Scalar
    #     ret = np.sqrt(radius_squared) < self.T1_radius # one element array
    #     if scint_num == 4:
    #         ret = np.sqrt(radius_squared) < self.T4_radius # one element array
    #     return ret
    def scint_condition(self, corner_pt, scint_radius, scint_num): # removing memory
        radius_squared = corner_pt[0]**2 + corner_pt[1]**2
        if scint_num == 4:
            return np.sqrt(radius_squared) < self.T4_radius
        return np.sqrt(radius_squared) < self.T1_radius
    ...
    #     D = -u if cond else u
    #     # Calculate the components of bigDelta
    #     bigDelta_x = o[0] - center[0]
    #     bigDelta_y = o[1] - center[1]
    #     # Calculate the squared magnitude of D
    #     magDsq = np.dot(D, D)
    #     # Calculate the squared magnitude of bigDelta
    #     magDeltasq = bigDelta_x**2 + bigDelta_y**2
    #     # Calculate the dot product of D and bigDelta
    ...
    def distance_circle(self, u, o, center, radius, quadrant=False): # remove variables once not needed
        # Calculate the dot product of u and o
        cond = np.dot(u, o) < 0
        # Calculate the normalized direction vector D
        D = -u if cond else u

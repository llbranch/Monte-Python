...
class Simulation:
    def __init__(self):
        ...
        print(
            "PARTICLE: Time Seperation between sequentual Particles if simulation more than 1 =",
            ...
            for Tbottom, Ttop in [(T1_z, T1_z + T1_width), (T4_z, T4_z + T4_width)]:
                ...
                if inside_scint:
                    while inside_scint:
                        ...
                        inside_scint = (z_1 <= (Ttop)) & (z_1 >= Tbottom)
        print(
            f"time elapsed in scintillators: {np.abs(times[1]-times[-1]):.2f}ps total scintillation points: {len(points[1:])}"
        )
        return (
    ...
    def run(self, *arg, **kwargs):
        ...
        for mult in range(num_particles):
            time_i, point_i, photon_i = self.particle_path(
            ...
            for i, (point_i, time_i) in enumerate(zip(points, times)):
                ...
                if point_i[2] >= self.T1z:
                    for photon in range(photons[i].astype(int)):
                        hit_PMT, travel_time, check = self.scintillator_monte_carlo(
                            point_i,
                            notabsorbed=True,
                    ...
                    # ELSE IN T4
                    for photon in range(photons[i].astype(int)):
                        hit_PMT, travel_time, _ = self.scintillator_monte_carlo(
                            point_i,
                            notabsorbed=True,
        ...
        self.signals_channelT1 = np.array(signals_channelT1) * self.q / 1e-12
    ...
    def to_csv(self, channels=2):
        ...
        if channels == 1:
            ...
            for time, signal, ch in zip(
                ...
                df["time"] = df["time"] / 1e12
                df.to_csv(
                    "monte_carlo_output_channel" + str(ch) + ".txt",
                    float_format="%.13f",
                    header=False,
                    index=False,
                    sep=" ",
